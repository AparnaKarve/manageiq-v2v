...
import { QUERY_V2V_OSP_TENANT_ATTRIBUTES, SET_V2V_INSTANCE_PROPERTIES_SINGLE_ROW, SET_V2V_INSTANCE_PROPERTIES_ROWS, QUERY_V2V_OSP_BEST_FIT_FLAVOR, SET_V2V_BEST_FIT_FLAVORS_IN_VM_ROWS } from './PlanWizardInstancePropertiesStepConstants';
...
export const setFlavorValues = (response, mappings, dispatch) => {
  ...
  response.data.results.forEach(result => {
    ...
    const tenantId = mappings.find(mapping => mapping.source_href_slug === `${vmsSlugPrefix}${vmId}`).destination_href_slug.slice(cloudTenantsSlugPrefix.length);

    vmBestFitFlavors.push({vmId: vmId, tenantId: tenantId, flavorId: flavorId});
  });

  dispatch({
    type: SET_V2V_BEST_FIT_FLAVORS_IN_VM_ROWS,
  ...
  dispatch({
    ...
    payload: new Promise((resolve, reject) => {
      API.post(url, postBody)
        .then(response => {
          resolve(response);
          setFlavorValues(response, mappings, dispatch);
        })
        .catch(e => {
          reject(e);
        });
...
export const setInstancePropertiesSingleRowAction = (row) => dispatch =>
  dispatch({
    type: SET_V2V_INSTANCE_PROPERTIES_SINGLE_ROW,
    payload: row
  });
